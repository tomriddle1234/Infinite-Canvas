"""ComfyUI 工作流与后端实例管理：/api/workflows/*, /api/comfyui/*。"""
...
from .. import comfyui, config, providers
from ..models import (
...
from . import generate as generate_route
...
def save_comfyui_instances(payload: ComfyInstancesPayload):
    ...
    try:
        providers.update_env_values({"COMFYUI_INSTANCES": ",".join(cleaned)})
    ...
    comfyui.reset_backend_local_load(cleaned)
    return {"instances": config.COMFYUI_INSTANCES}
...
def upload_workflow(payload: WorkflowUploadRequest):
    ...
    # 简单校验：API 格式（节点 id 为 key，含 class_type）
...
def run_workflow(name: str, payload: WorkflowRunRequest):
    ...
    return generate_route.generate(req)
